...
(function () {
   ...
   */
  var INVERT_LEFT_RIGHT = true;

  // Position-based move with hysteresis (normalized x in [0..1])
  var LEFT_ENTER = 0.45;
  var LEFT_EXIT = 0.52;
  var RIGHT_ENTER = 0.55;
  var RIGHT_EXIT = 0.48;
  var MOVE_REPEAT_MS = 160;

  // Wave down -> soft drop boost
  var DROP_DY_THRESHOLD = 0.10;
  var DROP_TIME_MS = 180;
  var SOFTDROP_REPEAT_MS = 80;
  var SOFTDROP_HOLD_MS = 650;

  // Wrist twist rotate
  var TWIST_TRIGGER_DEG = 25;
  var TWIST_RELEASE_DEG = 12;
  var ROTATE_DEBOUNCE_MS = 280;

  // Optional gestures
  var OPEN_PALM_TIP_MCP_MIN = 0.12, OPEN_PALM_HOLD_MS = 500, HARD_DROP_DEBOUNCE_MS = 800;
  var V_SIGN_HOLD_MS = 400, PAUSE_DEBOUNCE_MS = 800;
  ...
  function requestCamera() {
    if (cvManager.stream && cvManager.stream.active) return Promise.resolve(true);
